  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">이메일 계정 설정</h1>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 mb-6 border border-slate-200 dark:border-slate-700 shadow-card">
          <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-4">{editId ? '계정 수정' : '새 계정 추가'}</h2>
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all"
                >
                  직접 입력
                </button>
              </div>
      ...
      {accounts.length === 0 ? (
        ...
        <div className="space-y-3">
          {accounts.map(acc => (
            <div key={acc.id} className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-card">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-800 dark:text-slate-100">{acc.name}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      acc.is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'
                    }`}>
                      {acc.is_active ? '활성' : '비활성'}
                    </span>
                  </div>
                  <div className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{acc.email}</div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => openEdit(acc)}
                    className="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-lg transition-colors font-medium">
                    수정
                  </button>
                  <button onClick={() => confirm('삭제할까요?') && deleteMut.mutate(acc.id)}
                    className="text-xs text-slate-400 hover:text-red-500 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-lg transition-colors font-medium">
                    삭제
                  </button>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-xs text-slate-500 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                <div>
              ...
              <div className="flex gap-2 items-center">
                {acc.pop3_host && (
                  <button onClick={() => handleFetch(acc.id)} disabled={fetchStatus[acc.id] === 'loading'}
                    className="text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 disabled:opacity-50 text-slate-700 dark:text-slate-200 px-3 py-1.5 rounded-lg transition-colors font-medium">
                ...
                {fetchStatus[acc.id] && fetchStatus[acc.id] !== 'loading' && (
                  <span className="text-xs text-slate-500 dark:text-slate-400">{fetchStatus[acc.id]}</span>
                )}
              </div>
            </div>
          ))}
      ...
      {showCompose && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-lg border border-slate-200 dark:border-slate-700 shadow-xl">
            <h2 className="font-bold text-slate-900 dark:text-slate-100 mb-5">메일 작성</h2>
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">본문</label>
